...
namespace Redis.NET
{
    ...
    public class RedisSentinelManager : IDisposable
    {
        ...
        public void Add(string host) => Add(host, DefaultPort);
        ...
        {
            foreach (var _ in _sentinels.Where(sentinel => sentinel.Item1 == host && sentinel.Item2 == port).Select(sentinel => new { }))
            {
                return;
            }
        ...
        RedisSentinelClient Current() => new RedisSentinelClient(_sentinels.First.Value.Item1, _sentinels.First.Value.Item2);
        ...
        void OnConnectionConnected(object sender, EventArgs args) => Connected?.Invoke(this, new EventArgs());
    }
}